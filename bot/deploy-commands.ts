...
export async function deployCommands(): Promise<void> {
    try {
        ...
        const guildSpecificCommands = fullCommandList.filter(command => command.guildIds && command.guildIds.length > 0);
        ...
        if (guildSpecificCommands.length > 0) {
            ...
            for (const [guildId, commandsData] of guildCommandsMap) {
                try {
                    ...
                    logger.info(`Successfully deployed ${commandsData.length} guild-specific commands to guild: ${guildId}`);
                } catch (error) {
        ...
        logger.info(`Total commands deployed: ${fullCommandList.length} (${globalCommands.length} global, ${guildSpecificCommands.length} guild-specific)`);
    } catch (error) {
