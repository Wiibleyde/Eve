import { logger, streamRetriever } from "@/index";
...
export const data: SlashCommandOptionsOnlyBuilder = new SlashCommandBuilder()
    ...
    .addChannelOption((option) => option.setName("channel").setDescription("Salon où envoyer les notifications").setRequired(true))
    .addRoleOption((option) => option.setName("role").setDescription("Rôle à mentionner lors de la notification").setRequired(false))
...
export async function execute(interaction: CommandInteraction): Promise<void> {
    ...
    streamRetriever.addStream(streamer);

    await interaction.editReply({
        embeds: [successEmbed(interaction, `Le streamer ${streamer} a bien été ajouté à la liste des streams suivis`)],
    });
}